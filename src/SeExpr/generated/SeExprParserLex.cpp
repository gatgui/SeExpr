...
extern SeExpr_size_t SeExprleng;
...
static SeExpr_size_t SeExpr_n_chars;		/* number of characters read into SeExpr_ch_buf */
SeExpr_size_t SeExprleng;
...
SeExprYY_BUFFER_STATE SeExpr_scan_bytes (SeExprconst char *bytes,SeExpr_size_t len  );
...
#define SeExprYY_DO_BEFORE_ACTION \
	...
	SeExprleng = (SeExpr_size_t) (SeExpr_cp - SeExpr_bp); \
...
SeExpr_size_t SeExprget_leng (void );
 ...
 * we now use fwrite().
 */
#define ECHO fwrite( SeExprtext, SeExprleng, 1, SeExprout )
#endif
	...
	if ( SeExprYY_CURRENT_BUFFER_LVALUE->SeExpr_buffer_status == SeExprYY_BUFFER_EOF_PENDING )
		...
		while ( num_to_read <= 0 )
			...
			if ( b->SeExpr_is_our_buffer )
				{
				SeExpr_size_t new_size = b->SeExpr_buf_size * 2;
	...
	if ( *(SeExpr_c_buf_p) == SeExprYY_END_OF_BUFFER_CHAR )
		...
		if ( (SeExpr_c_buf_p) < &SeExprYY_CURRENT_BUFFER_LVALUE->SeExpr_ch_buf[(SeExpr_n_chars)] )
			...
			*(SeExpr_c_buf_p) = '\0';
			...
			{ /* need more input */
			SeExpr_size_t offset = (SeExpr_c_buf_p) - (SeExprtext_ptr);
			++(SeExpr_c_buf_p);
 ...
 * @return the newly allocated buffer state object.
 */
SeExprYY_BUFFER_STATE SeExpr_scan_bytes  (SeExprconst char * SeExprbytes, SeExpr_size_t  _SeExprbytes_len )
{
	SeExprYY_BUFFER_STATE b;
	char *buf;
	SeExpr_size_t n, i;
